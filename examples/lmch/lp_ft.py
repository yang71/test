...
from ggfm.data import args_print, open_pkl_file, open_txt_file, save_pkl_file, save_txt_file, ndcg_at_k, mean_reciprocal_rank
...
def get_lp_labels(graph):
    ...
    lp_labels = np.zeros([num_paper, lp_output_num])  # all_papers 546704
    for i in range(len(idxs)):
        for source_id in data_pairs[idxs[i]]:
            lp_labels[idxs[i]][cand_list.index(source_id)] = 1
    
    row_sums = lp_labels.sum(axis=1).reshape(-1, 1)
    lp_labels = np.where(row_sums != 0, lp_labels / row_sums, 0)
    return lp_labels


def lp_load_raw_data(data_dir, graph):
    ...
    print("lp_labels.shape: ", lp_labels.shape)
...
if __name__ == '__main__':
    ...
    cand_list = list(graph.edge_list['field']['paper']['PF_in_L2'].keys())  # field_id
    output_num = len(cand_list)

    data = lp_load_raw_data(args.data_dir, graph)
    ...
    for epoch in range(int(args.pretrain_epochs)+1):
        ...
        for batch in tqdm(train_loader):
            ...
            if step % args.eval_patience == 0:
                valid_losses, valid_ndcg, valid_mrr = evaluation(args, device, lm_tokenizer, model, args.batch_size, valid_idx, user_seq, hard_labels)
                ...
                if valid_ndcg > valid_ndcg_best:
                    valid_ndcg_best = valid_ndcg
